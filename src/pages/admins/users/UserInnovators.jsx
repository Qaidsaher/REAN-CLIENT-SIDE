...
import { getInnovators, deleteInnovator } from "../../../services/admins/innovatorService";

const UserInnovators = () => {
    ...
    const handleDelete = async () => {
        if (selectedUser) {
            try {
                await deleteInnovator(selectedUser._id);
    ...
    return (
        <AdminLayout selectedNav="innovators">
            <div className=" p-4 bg-gradient-to-br from-indigo-100 to-indigo-300 rounded-md shadow-md">
                {/* Header */}
                <div className="flex flex-col md:flex-row justify-between items-center mb-6">
                    <h2 className="text-4xl font-extrabold text-indigo-800 mb-4 md:mb-0">
                        Innovators Management
                    </h2>
                    <Link
                        to={`/admin/users/create?role=innovator`}
                        className="flex items-center px-4 py-2 text-white rounded-md shadow-md bg-indigo-600 text-white hover:bg-indigo-700 transition-all duration-300 transform hover:scale-105"
                    >
                        <FaPlus className="mr-2" /> Add New Innovator
                    </Link>
                </div>
                        ...
                        <tbody className="divide-y divide-gray-200">
                            ...
                            {users.map((user, index) => (
                                <tr key={user._id} className="hover:bg-indigo-50 transition">
                                    ...
                                    {/* Name */}
                                    <td className="p-3 font-semibold text-indigo-900">{user.firstName} {user.lastName}</td>
                                    ...
                                    <td className="p-3 flex items-center justify-center gap-4 text-lg">
                                        <Link
                                            to={`/admin/users/${user._id}?role=innovator`}
                                        ...
                                        <Link
                                            to={`/admin/users/edit/${user._id}?role=innovator`}
