...
// websiteService.js
import { getRandomInvestors } from "../services/websiteService";
import React, { useEffect, useState } from "react";
import { motion } from "framer-motion";
import { FaUserCircle } from "react-icons/fa";

// Investors Component using getInvestors API
const Investors = () => {
  ...
  useEffect(() => {
    const fetchInvestors = async () => {
      try {
        const data = await getRandomInvestors();
        setInvestors(data);
      } catch (error) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {investors.map((investor, idx) => (
            ...
            >
              ...
              <p className="mt-2 text-gray-600 text-center">{investor.bio}</p>
              <a
                href="/innovators"
                className="mt-6 px-6 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-full font-medium transition-all transform hover:scale-105 hover:shadow-lg"
              >
